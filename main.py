...
class Ui_FormPersonalAccount(object):
    ...
    def retranslateUi(self, Form):
        ...
        self.lineEditFilePath.setText(_translate("Form", "The path to the file to decode..."))
    ...
    def desEncode(self, text: str):
        result = self.des.encode(text, self.des.KEY)
        return result

    def desDecode(self, text: str):
        result = self.des.decode(text, self.des.KEY)
        return result

    def writeToEncodedFile(self, text: str, filePath: str):
        with open(filePath, 'w') as file:
            file.write(self.desEncode(text))
        file.close()
    ...
    def getTextFromEncodedFile(self, filePath: str):
        ...
        if os.path.exists(filePath):
            with open(filePath, 'r') as file:
                for line in file:
                    self.textEdit.setText(self.textEdit.toPlainText() + self.desDecode(line))
...
class Ui_MainWindow(object):
    ...
    def checkAccount(self, username: str, password: str):
        if self.checkDataForSpecialCharacters(username, password) is True:
            return False
        desLine = str(self.desEncode(username)) + ":" + str(self.desEncode(password))
        with open('users.txt', 'r+') as file:
            if desLine in file.read():
